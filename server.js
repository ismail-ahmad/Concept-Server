...
app.post('/add_new_client', async(req, res) => {
    ...
    res.status(200).json({message: 'new client added successfully!', ok: true, status: 200});
        ...
        res.json({ok: false, status: 500, message: 'error occurred while processing the request to the database', err});
    }
});

app.post('/add_user',  async(req, res) => {
    if (req.body.getCompany){
        try{
            const dbRes = await pool.query(`
                  SELECT client_name, id FROM clients
                `);
            res.status(200).json({ok: true, message: 'successful response', data: dbRes.rows, status: 200});
        } catch(err){
            res.json(err);
        }
    }
